...
- 创建：用户登录时，服务器生成一个唯一的session ID。
- 发送：服务器将这个session ID作为session cookie发送到用户的浏览器。
- 使用：浏览器在随后的每个请求中都会带上这个session cookie。
- 验证：服务器检查session cookie以验证用户的身份和授权状态。
- 销毁：一旦用户退出或会话超时，服务器将销毁对应的session，并指示浏览器删除session cookie。

## Referer

- 服务器可以检查 “Referer” 字段来确定请求是否来自于自己的网站。如果不是，可以拒绝提供资源，以此来防止资源被盗链。
